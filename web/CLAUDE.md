...
| State | Zustand (`useEditorStore`, `usePresetsStore`) |
| Persistence | IndexedDB via `idb` (`rewritebox-db` v1) |

## Architecture

### State Stores (`src/store/`)
- `editorStore.ts` — tabs array, activeTabId, CRUD actions for tabs
- `presetsStore.ts` — replace presets + AI prompt templates

### Core Logic (`src/lib/`) — pure functions, no side effects
- `replaceEngine.ts` — all find/replace logic (match positions, replace all, regex support)
- `promptBuilder.ts` — assembles AI prompt templates with `{{TEXT}}`/`{{INSTRUCTION}}` placeholders
- `db.ts` — `idb` database setup (stores: `tabs`, `presets`, `meta`)

### Hooks (`src/hooks/`)
- `useSessionPersistence.ts` — debounced (500ms) IndexedDB sync on store changes; restores state on mount
- `useFileIO.ts` — `Ctrl+S` saves `.txt` via `<a download>`, `Ctrl+O` opens file picker
- `useKeyboardShortcuts.ts` — global `document` keyboard listener
...
- The div renders `<mark>` elements at match positions via `useHighlight.ts`
...
3. **Phase 3** — Replace Presets (the core feature; includes default "Вы → Мы" preset)
4. **Phase 4** — Session persistence (IndexedDB) + file export/import
5. **Phase 5** — AI Prompt Builder (clipboard-based, no API calls)
6. **Phase 6** — Polish: command palette, status bar, distraction-free mode, production build
...
- Commit after each phase: `feat: Phase N — <description>`
