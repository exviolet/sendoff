...
- `useImageAttachment.ts` — картинка в промпте как **путь к файлу**. Два канала сходятся в
  один: диалог отдаёт путь уже существующего файла, буфер обмена — путь только что
  записанного (байты пишет Rust-команда `save_clipboard_image`). Дальше сборка промпта об
  источнике не знает. См. Gotcha №10 про буфер и `lib/attachments.ts` про вставку.
