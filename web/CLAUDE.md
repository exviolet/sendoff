...
- `attachments.ts` — вставка пути к картинке в текст промпта (пробелы по краям только там,
  где иначе путь склеится со словом). Голый путь, **без** markdown `![](…)`: терминальный
  агент markdown не рендерит.
- `platform.ts` — `isTauri`.
...
- `useImageAttachment.ts` — картинка в промпте как **путь к файлу**. Три канала сходятся в
  один: диалог и drag & drop отдают путь уже существующего файла, буфер обмена — путь только
  что записанного (байты пишет Rust-команда `save_clipboard_image`). Дальше сборка промпта об
  источнике не знает. Новых permissions требует **только** буфер: у брошенного файла путь уже
  приходит в `onDragDropEvent`, читать его вебвью не нужно. См. Gotcha №10 про буфер, №11 про
  регистрацию слушателя и `lib/attachments.ts` про вставку.
  > Брошенные картинки уходят **одной** вставкой, а не циклом: `insertPath` берёт позицию из
  > живой textarea, а каретка после `updateContent` доезжает лишь следующим кадром (RAF) —
  > цикл положил бы второй путь туда, где каретка стояла до первого. И картинки
  > обрабатываются **до** текстовых файлов: `addTabFromFile` заводит новый таб и делает его
  > активным, после чего путь уехал бы не в тот таб.
...
| `Ctrl+V` | **Редактор:** картинка из буфера → файл → путь в текст (текст вставляется как обычно) |
...
10. **Вставленную картинку WebKitGTK в событии `paste` не отдаёт.** Ни
    `clipboardData.files`, ни `clipboardData.items` не содержат её — проверялись оба, при
    скриншоте в буфере (`wl-paste --list-types` показывал ровно `image/png`). Работает
    только асинхронный `navigator.clipboard.read()`, и звать его надо **внутри** обработчика
    вставки: это жест пользователя, без него чтение буфера не разрешается. Привычный по
    Chrome/Firefox код «взять файл из DataTransfer» здесь молча не делает ничего — ровно так
    фича и не работала в первой версии. Замерено 2026-08-13, см. `hooks/useImageAttachment.ts`.

    Решение «перехватывать или нет» принимается по `clipboardData.types` **синхронно**: есть
    `text/*` — отдаём событие браузеру, нет — гасим и читаем буфер сами. После `await`
    DataTransfer уже выпотрошен, читать из него поздно.

11. **Слушатель, регистрируемый за `await`, обязан сниматься по флагу `cancelled`.** Cleanup
    эффекта возвращается немедленно, а `unlisten` появляется только после `await` — если
    эффект свернули в этот промежуток, cleanup видит `undefined`, отписаться не может, и
    слушатель того прохода живёт до конца сессии. Дальше **каждое** событие отрабатывает
    лишний раз.

    В `StrictMode` это не редкая гонка, а гарантия: React монтирует эффекты дважды на каждом
    монтировании. Так `onDragDropEvent` в `Editor.tsx` задваивал drop с самого начала — один
    брошенный `.md` открывал **два** таба, и это не замечали, пока тот же код не начал
    вставлять путь картинки: два одинаковых таба списываются на промах мышью, два одинаковых
    пути в строке — нет. В release-сборке `StrictMode` эффекты не задваивает, поэтому баг
    виден только в `bun dev` / `tauri dev`.

    Лечение — снимать слушатель на месте, если он доехал уже после сворачивания:
    ```ts
    ...
    ```
    Регистрация на уровне модуля (`themeStore.ts`) этой проблемы не имеет — она отрабатывает
    один раз за загрузку и cleanup'а не требует.
...
**Действия над строкой** (`TabSwitcher`: закрыть таб; `WorkspaceSwitcher`: правка):
- **Клавиша идёт через `onKeyDown` примитива и возвращает `true`**, иначе `Enter`/стрелки
  отработают вдобавок к действию. `F2` — правка, `Ctrl+Del` — удаление на месте.
  `Ctrl+Backspace` не трогать — это «удалить слово» в инпуте поиска.
- **Строка с кнопками — `<div>`, а не `<button>`**: кнопки не вкладываются друг в друга.
  `data-picker-index` переезжает на контейнер — доскроллу примитива важен атрибут, не тег.
  Счётчик держать крайним правым столбцом фиксированной ширины, а место под появляющуюся
  надпись занимать всегда (`invisible`) — иначе колонка ездит от выделения строки.

**CRUD внутри пикера — вторым видом той же модалки, а не диалогом поверх**
(`WorkspaceSwitcher`, по образцу `TriggerPhrasePicker`). Диалог поверх пикера пробовали —
он требует `disabled` на примитиве (оба слушают `keydown` в capture-фазе, пикер
зарегистрирован раньше и забирает `Enter`/`Esc` себе) и всё равно оставляет два наложенных
слоя. Второй вид дешевле и честнее. Что при этом обязательно:
- **`disabled: view !== "list"`** — в правке стрелки и `Enter` должны быть нативными (ввод
  в поле имени), а `Esc` обязан вернуть в список, а не закрыть модалку: свой слушатель.
- **Фокус в поиск при возврате** — примитив фокусирует `inputRef` только на маунте.
- **После удаления клампить `selectedIndex`** — примитив делает это только на `ArrowDown`,
  иначе курсор повиснет за концом укоротившегося списка.
- **Последствие удаления — текстом рядом с кнопкой**, а не в диалоге: читается ДО клика.
  Само удаление взводится вторым кликом (`Delete` → `Confirm delete`).
