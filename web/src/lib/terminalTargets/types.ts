...
export interface TerminalProvider {
  ...
  // Имя бинаря для диагностики. Из JS его иначе не узнать: в манифесте `name` entry
  // (`herdr-agent-list`) и сам `cmd` (`herdr`) — разные строки, а наружу плагин отдаёт
  // только первую. Doctor'у нужна вторая, чтобы поискать файл в PATH.
  //
  // Метода `diagnose()` здесь намеренно нет: все три провайдера падают одинаково, через
  // общий runScoped, поэтому классификация одна на всех и живёт в lib/diagnostics.
  // Метод появится, когда появится провайдер с другой моделью отказов.
  executable: string;
