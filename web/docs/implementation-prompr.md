...
You are an expert React + TypeScript developer. You will build **Sendoff** — a fast, minimal personal text editor as a local web application. The goal is a tool that opens instantly in a browser tab, lets the user paste any text, quickly find/replace, rewrite, and manage multiple documents — without the overhead of VS Code, Obsidian, or Notion.
...
1. Scaffold the project:
   ```bash
   bun create vite sendoff --template react-ts
   cd sendoff
   bun install
   bun add zustand idb
...
4. Create the main editor layout:
   ...
   - Header: app name "Sendoff", tab bar, action buttons
...
4. Import/Export all sessions:
   - "Export all" button: downloads `sendoff-backup.json` containing all tabs + presets
